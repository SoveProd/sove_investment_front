...
import type {
  JournalArticle,
  JournalPost,
} from "@/src/components/pages/Journal/types";
import {
  JournalRelatedCard,
  type JournalRelatedCardVM,
} from "@/src/components/sections/JournalRelated/JournalRelatedCard";
...
function toVM(item: JournalPost | JournalArticle): JournalRelatedCardVM {
  ...
  return {
    ...
    title: item.title,
    excerpt:
      item.excerpt ?? "Статья посвящается далее\nкраткое содержание описание",
    tags: item.category,
    date: item.date,
...
export function JournalRelatedSection({
  posts,
