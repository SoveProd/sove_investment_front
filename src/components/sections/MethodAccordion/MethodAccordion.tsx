              ...
              <div className="flex flex-col gap-6">
                {items.map((item) => (
                  <button
                    ...
                    className="w-full rounded-[32px] border border-black/10 bg-[#F7F5F2] text-left shadow-[0_20px_40px_rgba(0,0,0,0.04)] transition-all duration-300"
                  >
                        ...
                        {!!item.bullets?.length && (
                          <ul className="mt-6 space-y-3 text-[14px] text-black/55">
                            {item.bullets.map((bullet, index) => (
                              ...
                              >
                                <span className="mt-[8px] h-[4px] w-[4px] shrink-0 rounded-full bg-black/30" />
                                <span>{bullet}</span>
                              </li>
                            ))}
                          </ul>
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="absolute w-full rounded-[32px] border border-black/10 bg-[#F7F5F2] text-left shadow-[0_20px_40px_rgba(0,0,0,0.04)] transition-all duration-300"
                    >
                      <div className="grid grid-cols-12 gap-6 px-8 py-8">
                        <div className="col-span-7">
                          <h3 className="text-[24px] font-medium text-[#1f1f1f]">
                          ...
                          {!!item.bullets?.length && (
                            <ul className="mt-6 space-y-3 text-[14px] text-black/55">
                              {item.bullets.map((bullet, bulletIndex) => (
                                <li
                                  key={`${item.id}-preview-${bulletIndex}`}
                                  className="flex gap-3"
                                >
                                  <span className="mt-[8px] h-[4px] w-[4px] shrink-0 rounded-full bg-black/30" />
                                  <span>{bullet}</span>
